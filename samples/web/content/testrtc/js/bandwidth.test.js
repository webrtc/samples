...
.test('DataChannelThroughputTest', function(t, h) {
  ...
  function testDataChannelThroughput(config) {
    ...
    call.isGoodCandidate = h.checkRelay;
    ...
    function onMessageReceived(event) {
      ...
      if (now - lastBitrateMeasureTime >= 1000) {
        ...
        t.success('Transmitting at ' + bitrate + ' kbps.');
      ...
      if (stopSending && sentPayloadBytes == receivedPayloadBytes) {
        ...
        t.success('Total transmitted: ' + receivedKBits + ' kilo-bits in ' +
                      elapsedTime + ' seconds.');
        t.complete();
      }
    }
  ...
  h.asyncCreateTurnConfig(
    t, h, h.CEOD_URL, testDataChannelThroughput, t.fatal);
