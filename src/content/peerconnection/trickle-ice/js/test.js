...
test('Candidate Gathering', t => {
  ...
  driver.get(`${process.env.BASEURL ? process.env.BASEURL :
    (`file://${process.cwd()}`)}/src/content/peerconnection/trickle-ice/index.html`)
  .then(() => {
    t.pass('page loaded');
    return driver.findElement(webdriver.By.id('gather')).click();
  })
  .then(() => driver.wait(() => driver.executeScript(
      'return pc === null && candidates.length > 0;'), 30 * 1000))
  .then(() => {
...
test('Loading server data', {skip: process.env.BROWSER === 'firefox'},
  t => {
    ...
    .then(element => new webdriver.ActionSequence(driver).
