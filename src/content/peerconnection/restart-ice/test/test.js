...
'use strict';
export default {
  'Checking the state of buttons, video, and the changing of the candidate ID at during ICE restart.': (browser) => {
    ...
    browser.expect.element('#localCandidateId').to.have.value.that.equals(null);
    browser.expect.element('#callButton').to.not.be.enabled.before(1000);
    browser.expect.element('#restartButton').to.not.be.enabled.before(1000);
    ...
    browser.expect.element('#startButton').to.not.be.enabled.before(1000);
    browser.expect.element('#callButton').to.not.be.enabled.before(1000);
    browser.expect.element('#restartButton').to.be.enabled.before(1000);
