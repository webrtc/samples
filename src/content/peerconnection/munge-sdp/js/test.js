...
test('Munge SDP sample', t => {
  ...
  driver.get(`${process.env.BASEURL ? process.env.BASEURL :
    (`file://${process.cwd()}`)}/src/content/peerconnection/munge-sdp/index.html`)
  .then(() => {
    t.pass('page loaded');
    return driver.findElement(webdriver.By.id('getMedia')).click();
  })
  .then(() => driver.wait(() => driver.executeScript('return typeof window.localStream ' +
    '!== \'undefined\'')))
  .then(() => {
    t.pass('got media');
    return driver.findElement(webdriver.By.id('createPeerConnection')).click();
  })
  .then(() => driver.wait(webdriver.until.elementIsVisible(
      driver.findElement(webdriver.By.id('createOffer')))).click())
  .then(() => // Need to wait for createOffer to succeed which takes some time
  // on travis.
  driver.wait(() => driver.executeScript(
      'return document.querySelector(\'#local>textarea\').value !== \'\'')))
  .then(() => driver.findElement(webdriver.By.css('#local>textarea'))
      .getAttribute('value'))
  .then(value => {
  ...
  .then(() => // Need to wait for createAnswer to succeed which takes some time
  // on travis.
  driver.wait(() => driver.executeScript(
