...
test('Video width and video height are set on GUM sample', t => {
  ...
  driver.get(`${process.env.BASEURL ? process.env.BASEURL :
    (`file://${process.cwd()}`)}/src/content/getusermedia/gum/index.html`)
  .then(() => {
    t.pass('Page loaded');
  })
  .then(() => driver.wait(() => driver.executeScript('return window.stream !== undefined;'), 30 * 1000))
  // Check that there is a stream with a video track.
  .then(() => driver.executeScript('return stream && stream.getTracks().length'))
  .then(numberOfStreams => {
